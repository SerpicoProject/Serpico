...
class Reports
  ...
  property :associated_docx_template, String, length: 200
  property :associated_excel_template, String, length: 200
  ...
  #10_000 was not enough for heavy udv usage
  property :user_defined_variables, String, length: 1_000_000
...
class UserDefinedObjectTemplates
  ...
  property :udo_properties, Json, length: 10_000
end
...
class ExcelXslts
  include DataMapper::Resource
  property :id, Serial
  property :excel_location, FilePath, length: 400
  property :template_title, String, length: 400
  property :template_type, String, length: 400
  property :description, String, length: 400
  #hashmap, key = path in excel, value = path of corresponding xslt in serpico files
  property :xslt_sheet_locations, Json, length: 4000
  property :xslt_shared_strings_location, String, length: 400
end

class DocxXslts
  include DataMapper::Resource
  property :id, Serial
  property :docx_location, FilePath, length: 400
  property :description, String, length: 400
  property :xslt_location, String, length: 400
  property :template_title, String, length: 400
  property :template_type, String, length: 400
  property :screenshot_names, String, length: 10_000
  has n, :components, 'DocxXsltComponents',
      parent_key: [:id],
      child_key: [:docx_xslt_id]
end

# each docx xslt file is linked to its header/footer components
class DocxXsltComponents
  include DataMapper::Resource
  property :id, Serial
  property :xslt_location, FilePath, length: 400
  property :name, String, length: 400
  belongs_to :docx_xslt, 'DocxXslts',
             parent_key: [:id],
             child_key: [:docx_xslt_id],
             required: true
end
