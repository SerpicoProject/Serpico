...
get '/report/:id/attachments' do
  ...
  xslt = DocxXslts.first(template_title: @report.associated_docx_template)
  @screenshot_names_from_report = xslt.screenshot_names if xslt
  @screenshot_names_from_findings
...
post '/report/:id/user_defined_variables' do
  ...
  data.each do |k, v|
    if k =~ /variable_data/ && (v !~ /\<paragraph\>/) && !v.empty?
...
get '/report/:id/status' do
  ...
  if xslt_elem
    ...
    if docx_xml.to_s =~ /\[!!/
      ...
      content_types = read_archive(rand_file, '[Content_Types].xml')
      ...
      archive_modify(rand_file, content_types, '[Content_Types].xml')
...
#:template_type is the type of file to be generated
# it can be equal to : finding, report, status, summary
# Word files are : finding, report, status
# Excel files are : summary
get '/report/:id/generate/:template_type' do
  id = params[:id]
  doc_to_generate = params[:template_type]
  supported_template_types = %w[finding report status summary]
  unless supported_template_types.include? doc_to_generate.downcase
    return "Wrong Template Type : #{doc_to_generate}"
  end
  # Query for the report
  @report = get_report(id)
  ...
  finding_number = 1
  if doc_to_generate == 'finding'
    @findings = Findings.all(report_id: id, id: params[:finding_id])
    return "Given Finding ID doesn't exist !" if @findings.empty?
  else
    @findings, @dread, @cvss, @cvssv3, @risk, @riskmatrix = get_scoring_findings(@report)
  end
  ...
  if (msfsettings = RemoteEndpoints.first(report_id: @report.id))
    if (rpc = msfrpc(@report.id))
      ...
      res = rpc.call('db.services', limit: 100_000, only_up: true)
  ...
  if doc_to_generate == 'report'
    xslt_elem = DocxXslts.first(template_title: @report.associated_docx_template)
    document_name = "#{@report.report_name}.docx"
  elsif doc_to_generate == 'status'
    xslt_elem = DocxXslts.first(template_type: 'Word - Status Template')
    document_name = "#{@report.report_name} - Status.docx"
  elsif doc_to_generate == 'finding'
    xslt_elem = DocxXslts.first(template_type: 'Word - Finding Template')
    document_name = @findings[0].title
  end
  if doc_to_generate == 'report' || doc_to_generate == 'status' || doc_to_generate == 'finding'
    unless xslt_elem
      return "#{doc_to_generate} template not found :-). IF you're an admin, create one <a href='/admin/templates/add'>here</a>!"
    end
    # Push the finding from XML to XSLT
    xslt = Nokogiri::XSLT(File.read(xslt_elem.xslt_location))
    ...
    # We use a temporary file with a random name
    rand_file_name = "./tmp/#{rand(36**12).to_s(36)}.docx"

    # Create a temporary copy of the word doc
    FileUtils.copy_file(xslt_elem.docx_location, rand_file_name)
    ...
    if docx_xml.to_s =~ /\[!!/
      # first we read in the current [Content_Types.xml]
      content_types = read_from_zip(rand_file_name, '[Content_Types].xml')
      ...
      archive_modify(rand_file_name, content_types, '[Content_Types].xml')
      ...
      imgs.each do |image_i|
        ...
        # search for the image in the attachments
        image = Attachments.first(report_id: id, conditions: ['lower(description) = ?', name.downcase])

        # tries to prevent breakage in the case image dne
        if image
          # inserts the image
          docx = image_insert(docx, rand_file_name, image, end_xml)
        else
          docx << end_xml
        end
      end
    ...
    rels_file =  read_from_zip(rand_file_name, 'word/_rels/document.xml.rels')
    # Noko syntax rels
    ...
    write_to_zip(rand_file_name, 'word/_rels/document.xml.rels', content_to_write)
    # Update hyperlinks
    docx = hyperlinks['xmlText']
    # put the created document.xml in the archive that will become the docx
    archive_modify(rand_file_name, docx, 'word/document.xml')

    list_components.each do |name, xml|
      # put the created header/footer in the archive that will become the docx
      archive_modify(rand_file_name, xml.to_s, name)
    end

    serpico_log("Report generation attempted, Report Name: #{document_name}")
    send_file rand_file_name, type: 'docx', filename: "#{document_name}.docx"

    #################################### EXCEL GENERATION PART ###########################

  elsif doc_to_generate == 'summary'
    # xslt_elem = DocxXslts.first(template_title: @report.associated_docx_template)
    excel_xslt = ExcelXslts.first(template_title: @report.associated_excel_template)
    if excel_xslt.nil?
      return "You didn't configure which Excel Template to use :-)... Go to <a href='/report/#{@report.id}/edit'>here</a> to fix this."
    end

    # hackish stuff to have newlines
    report_xml = report_xml.gsub('</paragraph>', 10.chr).gsub('<paragraph>', '')
    # hackish stuff to have some kind of bullet transformation
    report_xml = report_xml.gsub('<bullet>', '=>').gsub('</bullet>', '')

    # This file will store the excel sent to the user
    rand_file_name = "./tmp/#{rand(36**12).to_s(36)}.xlsx"
    FileUtils.copy_file(excel_xslt.excel_location, rand_file_name)

    ##### sharedstrings generation part

    xslt_to_transform = Nokogiri::XSLT(File.read(excel_xslt.xslt_shared_strings_location))
    shared_strings_xml = xslt_to_transform.transform(Nokogiri::XML(report_xml))
    archive_modify(rand_file_name, shared_strings_xml.to_s, 'xl/sharedStrings.xml')

    ###### worksheets generation part

    worksheets = excel_xslt.xslt_sheet_locations
    ...
    worksheets.each do |archive_path, xslt_path|
      ...
      previous_index = ''
      # we start reparing at the first indexed row
      i = worksheet_to_repair_xml.at_xpath('//xmlns:row')['r'].to_i
      worksheet_to_repair_xml.xpath('//xmlns:row').each do |row|
        original_index = row['r']
        # if the current row was duplicated because of an xsl loop, we fix the row and cells index
        if original_index.to_i <= previous_index.to_i
          # we assign the new value for the fixed row
          new_row_index = i
          row['r'] = new_row_index
          row.xpath('xmlns:c').each do |c|
            original_cell_index = c['r']
            letter_part = original_cell_index.tr('0-9', '')
            new_cell_index = letter_part + i.to_s
            # we assign the new value for the fixed cell
            c['r'] = new_cell_index
          end
          i += 1
          previous_index = new_row_index
          next
        end
        i += 1
    ...
    # we send the resulting file to the user
    send_file rand_file_name, type: 'xlsx', filename: "#{@report.report_name} - Summary.xlsx"
...
get '/report/:id/presentation_export' do
  ...
  File.open(rand_file_name, 'w') do |f|
    f.write htmlDoc
  end
  ...
  Zip::File.open(rand_zip, Zip::File::CREATE) do |zipfile|
    zipfile.add('presentation.html', rand_file_name)
